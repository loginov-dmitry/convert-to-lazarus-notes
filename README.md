...
По умолчанию Lazarus использует виджеты GTK2. К сожалению, они устарели примерно 20 лет назад, с ними существует множество проблем, связанных с некорректной отрисовкой стандартных элементов управления. Примерно 15 лет назад начались работы по поддержке виджетов GTK3 (сейчас они также безнадёжно устарели), однако на момент 2024г эти работы всё ещё не закончены. 

Гораздо лучше обстоят дела с виджетами QT5 (по сравнению с GTK2 качество отрисовки стандартных компонентов на QT5 несопоставимо лучше, однако не все сторонние компоненты поддерживают эти виджеты). На данный момент виджеты QT5 считаются устаревшими, проводится работа по поддержке виджетов QT6. В данной статье речь пойдёт (к сожалению) о работе с GTK2.
