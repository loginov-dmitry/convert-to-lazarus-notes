...
<!-- 
Немного рекламы своих решений: SmartHolder, LDSLogger, TimeIntervals
Про интерфейс IDataSet
Про сборку deb-пакета
Про необходимость добавления в группу для работы com-порта
Про особенности настройки прав доступа в каталоге /opt/
Про разработку служб
-->
...
Пересобрать Лазарус с виджетами QT5 можно несколькими способами. Один из способов: 
...
**Внимание!** Для сложных форм может потребоваться ручная доработка lfm-файла (либо конвертера). Вы легко узнаете о необходимости такой доработки, если попытаетесь открыть lfm-файл в Лазарусе: IDE сообщит о всех незнакомых компонентах и свойствах.
...
Кроме того, я предложил вариант разметки файла "anchoreditor.lfm", который позволяет встроить редактор привязки в правой части главного окна IDE Lazarus (решена проблема слишком узких компонентов TComboBox).
...
Для решения этой задачи вы можете воспользоваться модулем [UniqueInstance](https://wiki.lazarus.freepascal.org/UniqueInstance), который можно установить с помощью OnlinePackageManager. Здесь же я покажу, как для этой задачи использовать модуль `MutexObjectUnit`. Код будет кроссплатформенным (работает и в Windows и в Linux).
...
## Отладчик в Lazarus <a name="laz-debugger"></a>

В 2024г наблюдался потрясающий прогресс в области разработки собственного отладчика, поставляемого в комплекте с IDE Lazarus (прогресс пока ограничен веткой "trunk" Лазаруса). Теперь можно забыть про старый тормозной отладчик gdb. Выполнено огромное количество различных улучшений, исправлений, в том числе в области визуализации информации по отлаживаемому коду (огромное спасибо за эту работу Александру Багелю). Устранены проблемы с производительностью отладчика в Линуксе (также спасибо Александру). Отладчик теперь умеет отлаживать в Линуксе не только исполняемый модуль, но и so-библиотеки, но для этого существует обязательное требование - отладочную информацию (например, в формате "Dwarf 3"), необходимо встраивать одновременно и в исполняемый модуль и в so-библиотеку. Для меня лично удобнее видеть отладочную информацию в отдельном файле, но так отладка so-библиотек работать не будет.

На 2024г самым большим недостатком отладчика для меня является отсутствие возможности просмотра значений свойств, для которых требуется вызов функции.

## Генерация map-файлов с отладочной информацией <a name="laz-debug-map"></a>

При необходимости Лазарус умеет генерировать map-файл с отладочной информацией. Lazarus никак этот map-файл не использует, однако он может оказаться полезным для сторонних решений, например для системы защиты ПО Guardant Station. Для генерации map-файла необходимо включить опцию "-Xm" в списке директив компиляции для проекта (раздел "Compiler options" / "Custom options").

## Отладка IDE Lazarus <a name="laz-debug"></a>

Если при работе в IDE Lazarus вы столкнулись с ошибкой, то, если она воспроизводится, вы можете самостоятельно её исправить. Для этого следует запустить Lazarus, открыть в нём проект "lazarus\ide\lazarus.lpi" и запустить второй экземпляр Лазаруса под отладчиком с помощью кнопки F9. В этом случае первый экземпляр Лазаруса запустит под отладчиком второй экземпляр. Если при работе отлаживаемого экземпляра возникнет исключение, то отладчик должен показать точку, в которой оно возникло. 

Ошибка может оказаться либо в модулях самого Лазаруса, либо в модулях одного из пакетов. Если вы исправили ошибку в модуле какого-либо пакета, то необходимо будет сперва перекомпилировать этот пакет, а затем пересобрать Lazarus (меню Tools -> Build Lazarus). Если вы исправили ошибку непосредственно в коде Lazarus, до достаточно его пересобрать, предварительно закрыв отлаживаемый экземпляр.

Для того, чтобы определить, в каком пакете находится тот или иной модуль, необходимо навести мышкой на названии модуля. Например, откройте модуль "LConvEncoding" и в строке "unit LConvEncoding" наведите мышкой на названии модуля "LConvEncoding", появится подсказка с названием пакета "LazUtils".

Для того, чтобы перекомпилировать пакет, щелкните меню Package -> Open loaded package, найдите нужный пакет, выберите его и нажмите Open. После этого нажмите Compile.

**Внимание!** Данная информация не относится к пакетам из состава FPC (у меня не было опыта внесения исправлений в модули RTL).

## Определение имени пользователя и компьютера в Linux  <a name="laz-user-name"></a>

Практически в каждой программе возникает необходимость определить имя пользователя, запустившего программу, а также наименование компьютера.

Имя пользователя можно получить с помощью кода:

```pascal
UserName := GetEnvironmentVariable('USER');
```

Наименование компьютера можно получить с помощью кода:

```pascal
ComputerName := Unix.GetHostName;
```

## Особенности блокирующих операций в Linux <a name="laz-blocking-funct"></a>

Существует множество операций, при выполнении которых производится временная блокировка потока, который вызвал соответствующую функцию. Такие операции называются блокирующими. Например, функция `FileRead` является блокирующей. Она может приостановить поток на неопределённое время, пока в устройстве ввода не появятся данные, которые можно записать в буфер Buffer. В случае чтения из файла она дожидается ответа от накопителя (ресурсы процессора на такое ожидание не тратятся).

В Линуксе программе может быть послан сигнал прерывания ESysEINTR=4 (Interrupted system call), который прервёт работу функции `fpRead` с кодом ответа -1 (при этом fpgeterrno=ESysEINTR). Такой сигнал прерывания не должен нарушить работоспособность программы, но его следует корректно обработать и вызвать блокирующую функцию повторно. Пример такой обработки представлен ниже:
...
## Особенности линковки библиотек, представленных файлом *.a <a name="laz-linker-option"></a>

Если вам требуется слинковать с вашим ПО библиотеку, написанную на C/C++ и представляющую собой архив (`*.a`) с объектыми файлами, то необходимо использовать директиву `{$L filename.a}`. Если библиотека написана на C++, то вызывать можно будет только те функции, которые экспортируются с использованием директивы `extern "C"`, поскольку в этом случае не искажаются названия функций. 
На этапе линковки в ОС Linux запускается встроенная утилита линковки "ld", которая анализирует код в объектных файлах и, если в них обнаружена зависимость от других so-библиотек, то будет выдана ошибка линковки, если в опциях проекта не перечислены названия этих библиотек. Например, пусть в объектном файле имеется зависимость от библиотеки "librt.so" (одна из стандартных библиотек в Linux). Для успешной линковки библиотеки необходимо открыть окно "Project options", перейти в раздел "Compilation and linking" и в разделе "Linking" включить флаг "Pass options..." и указать ключ "-lrt". Префикс "lib" и расширение файла указывать не нужно. Поиск so-библиотеки выполняется в стандартных каталогах Linux.

## Настройка прав для работы с COM-портами в Linux <a name="com-ports"></a>

Для того, чтобы программа могла работать с COM-портами, она должна быть запущена под пользователем, который является участником группы "dialout" (по крайней мере дистрибутивах, основанных на debian). Для включения пользователя testuser в группу dialout следует выполнить команду:

```
sudo usermod -a -G dialout testuser
```

## Настройка доступа к файлам и каталогам в Linux  <a name="file-acl"></a>

Для каждого файла/каталога Linux хранит владельца (u) и группу (g). Изменить владельца/группу можно с помощью команды:

```
sudo chown username:groupname filename
```

В этом примере для файла `filename` назначаются одновременно владелец `username` и группа `groupname`. При необходимости можно изменить только владельца либо тольку группу.

Для настройки доступа к файлам и каталогам в первую очередь используют команду `chmod`, с помощью которой вы можете для файла настроить разрешения на запись(w)/чтение(r)/выполнение(x) раздельно для владельца(u) / группы(g) / остальных пользователей (o). Например:

```
sudo chmod u+rwx,g+rwx,o+r-w-x MyFile
```

В этом примере даётся полный доступ (включая разрешение на запуск) к файлу MyFile для владельца (u) и группы (g), но для остальных пользователей (o) разрешено только чтение файла.

Рекомендую изучить команды `chown` и `chmod` более подробно в Интернете.

Механизм настройки прав доступа к файлам и каталогам с помощью `chmod` имеет серьёзные ограничения и я рекомендую рассмотреть более продвинутые команды, управляющие списками контроля доступа (acl).

Для внесения изменений в списки контроля доступа используется команда `setfacl`, а для чтения информации из списков используется `getfacl`.

Благодаря команде `setfacl` вы можете настроить права доступа к файлу для множества пользователей и групп, например, у пользователей user1 и user2 будут полные права, у пользователей user3 и user4 будут права только для чтения, а у пользователей user5 и user6 вообще не будет никаких прав доступа к файлу. Это вроде элементарные вещи, но с помощью команды `chmod` такую настройку выполнить очень сложно (а зачастую невозможно).

Пример настройки для групп пользователей user1..user6 с использованием команды `setfacl`:

```
# Для групп пользователей user1 и user2 предоставляем полный доступ
sudo setfacl -m group:user1:rw /tmp/myfile
sudo setfacl -m group:user2:rw /tmp/myfile

# Для групп пользователей user3 и user4 разрешаем только чтение
sudo setfacl -m group:user3:r /tmp/myfile
sudo setfacl -m group:user4:r /tmp/myfile

# Для групп пользователей user5 и user6 запрещаем доступ к файлу
sudo setfacl -m group:user5:0 /tmp/myfile
sudo setfacl -m group:user6:0 /tmp/myfile
```

Обратите внимание на то, что в этом примере права заданы для группы, а не для пользователя. Дело в том, что при добавлении нового пользователя в Linux автоматически создаётся группа с таким же именем, как у пользователя. Настраивать права доступа для групп намного удобнее, чем для конкретных пользователей (удобство проявляется в упрощении администрирования).

Если вы хотите задать права на чтение и запись для каталога `/opt/myproj` и всех имеющихся в нём подкаталогов, то используйте команду:

```
find /opt/myproj -type d -exec sudo setfacl -m group:mygroup:rwx {} \;
```

Здесь команда `find` найдет каталог `/opt/myproj` и все подкаталоги и для каждого вызовет команду "sudo setfacl -m group:mygroup:rwx {}", а вместо "{}" будет подставляться путь к найденному каталогу. Буква "x" означает, что можно получить содержимое каталога (список имён файлов и подкаталогов).

В этом примере назначение прав работает только для существующих каталогов. Если вы внутри каталога `/opt/myproj` вручную создадите новый каталог, то для него эти права действовать не будут. Для устранения этого недостатка используйте команду:

```
find /opt/myproj -type d -exec sudo setfacl -m default:group:mygroup:rwx {} \;
```

Здесь добавлено ключевое слово "default:", благодаря которому указанное разрешение будет автоматически устанавливаться для новых подкаталогов.

Быстро настроить права доступа к файлам, находящимся в каталоге `/opt/myproj` и его подкаталогах можно с помощью команды:

```
find /opt/myproj -type f -exec sudo setfacl -m group:mygroup:rw {} \;
```

## Особенности разработки служб с помощью Lazarus  <a name="laz-services"></a>

В Интернете имеется материал по разработке служб в IDE Lazarus. В принципе ничего сложного нет, но есть ряд особенностей.

В составе Lazarus присутствует пример службы "tdaemon" (lazarus\examples\TDaemon\). В данном примере предлагается динамически создавать файл описания службы servicename.service, однако на практике такой способ работает плохо, поскольку для регистрации службы предлагается запустить программу с правами root. Считаю, что прав root для запуска приложения - слишком много, это будет создавать проблемы при администрировании (ни один грамотный администратор не разрешит запускать малоизвестный сторонний софт с правами root). Рекомендую заранее разработать файл описания службы и скопировать его в каталог `/lib/systemd/system/` (например, при установке вашего ПО). 

Пример содержимого файла описания службы (myprojservice.service):
...
Type=simple
User=myproj-service
ExecStart=/opt/myproj/myprojservice -run

[Install]
WantedBy=multi-user.target
```

Обратите внимание на строку "User=myproj-service". Служба должна запускаться не от обычного пользователя, который может работать за компьютером, а от имени системного пользователя. У системного пользователя, в отличии от обычного пользователя, отсутствует пароль, под ним могут запускаться службы, а у оператора, работающего за компьютером, отсутствует возможность выполнить вход от имени системного пользователя. Благодаря этому обеспечивается высокий уровень безопасности. Однако есть и минусы! Если вы разрабатываете HTTP-сервер, то он сможет открыть TCP-порт с номером, начиная с 1025. Первые 1024 порта считаются привилегированными, для их использования требуются права `root` либо дополнительные шаги по администрированию.

Для создания системного пользователя "myproj-service" используйте команду: 

```
sudo useradd -r -d /opt/myproj -M -s /usr/sbin/nologin myproj-service
```

Для того, чтобы выполнить запуск службы, необходимо скопировать файл описания службы `myprojservice.service` в каталог `/lib/systemd/system/`, а после этого выполнить команды:
- `sudo systemctl daemon-reload` - для того, чтобы сообщить Linux о появлении новой службы:
- `sudo systemctl start myprojservice.service` для ручного запуска службы.

Для автоматического запуска службы (при включении компьютера) потребуется дополнительная команда:

`sudo systemctl enable myprojservice.service`

Для того, чтобы остановить службу, используйте команду: `sudo systemctl stop myprojservice.service`

Если при включении компьютера ваша служба не запускается, то используйте команду ручного запуска службы из командной строки. Linux постарается отобразить подробную информацию об ошибке запуска службы и вы сможете очень быстро разобраться с причиной ошибки.

**Внимание!** Убедитесь, что в проекте приложения-службы отсутствуют модули, имеющие отношения к виджетам (визуальным компонентам), например Forms, Buttons, Controls, Dialogs и т.п. Если в файле проекта (.lpr) указан модуль `Interfaces`, то необходимо его удалить. Дело в том, что для запущенной службы графическое окружение недоступно, библиотеки виджетов (gtk2, qt5, qt6) не загружены и при попытке любого обращения к ним будет возникать ошибка. К сожалению, Lazarus мало чем сможет помочь для выявления модулей, имеющих отношения к GUI. При компиляции программы будет выдаваться ошибка линковщика:

```
Warning: linker: /usr/bin/ld: //home/username/lazarus/lcl/units/x86_64-linux/wsimglist.o: in function `REGISTERCUSTOMIMAGELISTRESOLUTION':
```

Однако название лишнего модуля и место, где он подключен, не сообщается. Вам придётся самостоятельно выискивать все такие модули. В моём случае было потрачено несколько рабочих дней, поскольку используется одинаковая кодовая база и для графического приложения и для службы.

В IDE Lazarus имеется механизм, который позволяет найти проблемный модуль. Для этого:
1) щелкните меню View -> Unit Dependencies;
2) нажмите кнопку "Refresh" (Обновить);
3) в левой части окна найдите модуль "interfacebase.pp" (в разделе LCLBase);
4) в правой части окна найдите раздел "used by interface", там найдите "forms.pp", затем снова "used by interface", затем "buttons.pp", затем снова "used by interface" и там будет отображаться проблемный модуль, в котором в разделе interface в строке "uses" указан модуль "Buttons", который ссылается на модуль "Forms". Модуль "Buttons" здесь указан для примера. У вас могут быть другие варианты.

## Скрипт установки библиоеки libQt5Pas.so.1.2.xx и создания/исправления симлинка  <a name="libQt5Pas-install"></a>

Если вы планируете распространять своё ПО для дистрибутивов Linux, основанных на QT5, до скорее всего будет достаточно включить в свой инсталляционный пакет/архив библиотеку "libQt5Pas.so.1.2.15" (либо libQt5Pas.so.1.2.16) (потребуется её скопировать в папку "/usr/lib/x86_64-linux-gnu/" и создать/исправить симлинк "libQt5Pas.so.1", все остальные библиотеки скорее всего уже имеются в составе данной ОС (в любом случае следует проверять).

Однако, если Linux не основан на QT5, то в нём может не оказаться необходимых библиотек. В этом случае потребуется установить пакет libqt5pas1. Благодаря этому будут автоматически установлены все остальные пакеты, от которых зависит пакет libqt5pas1 (т.е. будут установлены необходимые so-файлы с виджетами qt5). Кроме этого, пакет libqt5pas1 установит файл  libQt5Pas.so.1.2.XX какой-нибудь старой версии и создаст на него симлинк "libQt5Pas.so.1". Наша задача включить в дистрибутив нашего ПО актуальный файл (например, libQt5Pas.so.1.2.16), скопировать его в папку "/usr/lib/x86_64-linux-gnu/" и исправить симлинк "libQt5Pas.so.1", чтобы он указывал на самую актуальную версию библиотеки "libQt5Pas.so.1.2.XX".

Ниже приведён пример скрипта (например, postinst), который автоматически скопирует библиотеку "libQt5Pas.so.1.2.16" в системный каталог и создаст/исправит симлинк "libQt5Pas.so.1".

```bash
#!/bin/bash

# APP_DIR - каталог, в который инсталлятор предварительно установил файл libQt5Pas.so.1.2.16
APP_DIR=/opt/my_app

echo "Проверка версии симлинка libQt5Pas.so.1"

# Путь к файлу и симлинку
LIB_DIR="/usr/lib/x86_64-linux-gnu"
TARGET_FILE_PAS="$LIB_DIR/libQt5Pas.so.1.2.16"
SYMLINK_PAS="$LIB_DIR/libQt5Pas.so.1"
SOURCE_FILE_PAS="$APP_DIR/libQt5Pas.so.1.2.16"

# Копируем файл libQt5Pas.so.1.2.16, если его нет
if [ ! -f "$TARGET_FILE_PAS" ]; then
    echo "Файл $TARGET_FILE_PAS отсутствует в системе. Копируем его..."
    cp "$SOURCE_FILE_PAS" "$LIB_DIR/"
else
    echo "Файл $TARGET_FILE_PAS уже существует в системе."
fi

# Получаем имя самого актуального файла libQt5Pas.so.* в системном каталоге
NEWER_FILE_PAS=$(find "$LIB_DIR" -name "libQt5Pas.so.1.*" | sort -V | tail -n 1)

echo "Самый новый файл в системе: $NEWER_FILE_PAS"

# Обновляем / создаем симлинк libQt5Pas.so.1
ln -sf "$TARGET_FILE_PAS" "$SYMLINK_PAS"
echo "Создан / обновлён симлинк $SYMLINK_PAS"
```

## Скрипт корректировки симлинков libncurses.so.5 и libtommath.so.0  <a name="libncurses"></a>

Наше ПО использует Firebird 3.0. Более старые версии (например, Firebird 2.5) использовать на Линуксе опасно (проблемы совместимости с некоторыми системными библиотеками). Более новые версии мы пока не используем (требуется дополнительная адаптация нашего ПО).
Для того, чтобы СУБД Firebird 3.0 мог работать в Линуксе, ему требуются дополнительные пакеты libtommath1 и libncurses6,
а также симлинки "libtommath.so.0" и "libncurses.so.5" (в системном каталоге).
Мы можем установить Firebird 3.0 из репозитория Linux (все нужные пакеты будут установлены автоматически), однако в официальном репозитории Linux скорее всего окажется очень старая версия данной СУБД. Лучше устанавливать Firebird 3.0 из архива, который доступен на официальном сайте.

Несмотря на то, что Firebird 3.0 требует симлинк "libncurses.so.5" от пакета libncurses5, он вполне работает и с более современным пакетом libncurses6, который в Линуксе уже может быть установлен.

Ниже представлен пример скрипта для корректировки симлинков "libncurses.so.5" и "libtommath.so.0".
Скрипт учитывает различные версии so-библиотек.
...
# Создание симлинка для libncurses
if [ -f "$LIB_DIR/libncurses.so.6.4" ]; then
    ln -sf "$LIB_DIR/libncurses.so.6.4" "$LIB_DIR/libncurses.so.5"
    echo "Создан / обновлён симлинк $LIB_DIR/libncurses.so.5 -> libncurses.so.6.4"
elif [ -f "$LIB_DIR/libncurses.so.6.1" ]; then
    ln -sf "$LIB_DIR/libncurses.so.6.1" "$LIB_DIR/libncurses.so.5"
    echo "Создан / обновлён симлинк $LIB_DIR/libncurses.so.5 -> libncurses.so.6.1"
else
    echo "Ошибка: ни libncurses.so.6.4, ни libncurses.so.6.1 не найдены в $LIB_DIR"
fi

# Создание симлинка для libtommath
if [ -f "$LIB_DIR/libtommath.so.1.2.1" ]; then
    ln -sf "$LIB_DIR/libtommath.so.1.2.1" "$LIB_DIR/libtommath.so.0"
    echo "Создан / обновлён симлинк $LIB_DIR/libtommath.so.0 -> libtommath.so.1.2.1"
elif [ -f "$LIB_DIR/libtommath.so.1.1.0" ]; then
    ln -sf "$LIB_DIR/libtommath.so.1.1.0" "$LIB_DIR/libtommath.so.0"
    echo "Создан / обновлён симлинк $LIB_DIR/libtommath.so.0 -> libtommath.so.1.1.0"
else
    echo "Ошибка: ни libtommath.so.1.2.1, ни libtommath.so.1.1.0 не найдены в $LIB_DIR"
fi
```

## Поддержка тёмной темы в Linux <a name="dark-theme"></a>

Современные приложения должны учитывать предпочтения пользователей, включая выбор между светлой и тёмной темой интерфейса. В этой части мы рассмотрим, как реализовать поддержку тёмной темы в приложениях под Linux.

Виджеты QT5 по умолчанию поддерживают тёмную тему в Linux (например, в Astra Linux и RedOS). Виджеты GTK2 также корректно работают с тёмной темой без дополнительных настроек (в зависимости от дистрибутива Linux). Если вы используете только стандартные компоненты и системные цвета, то проблем с поддержкой тёмной темы возникнуть не должно.

Однако при разработке сложных приложений системных цветов может быть недостаточно. В таких случаях приложение должно уметь динамически определять текущую тему (светлую или тёмную) и адаптировать цвета интерфейса.

### Определение текущей темы

Для определения текущей темы можно использовать следующую функцию:
...
begin
  // Сравниваем яркость текста и фона окна
  Result := ColorToGray(clWindowText) > ColorToGray(clWindow);
end;
```

Эта функция является кроссплатформенной и работает с любыми виджетами. Однако в консольных приложениях её результат может быть некорректным (скорее всего, всегда False).

### Конвертация цветов

Для каждого цвета светлой темы можно вручную подобрать соответствующий цвет для тёмной темы. Ниже приведён пример функции `ConvertGUIColor`, которая выполняет такую конвертацию:

```pascal
function ConvertGUIColor(ColorForLightTheme: TColor): TColor;
begin
  Result := ColorForLightTheme;
  if CurrentThemeIsDark then
  begin
    if (ColorForLightTheme = clBlue)  then
      Result := $00B5B125
    else if (ColorForLightTheme = clGreen) then
      Result := $007CAF82
    else if ColorForLightTheme = clRed then
      Result := $007167DD
    else if ColorForLightTheme = clWhite then
      Result := clBlack
    else if ColorForLightTheme = clBlack then
      Result := clWhite
  end;
end;
```

Этот пример можно доработать, добавив больше цветовых пар.

### Использование MetaDarkStyle

Для упрощения работы с тёмной темой можно использовать пакет MetaDarkStyle (https://github.com/zamtmn/metadarkstyle), доступный через OnlinePackageManager. Он позволяет:

• Добавить поддержку тёмной темы для приложений под Windows.

• Переключить IDE Lazarus в режим "тёмная тема" для тестирования интерфейса.

Поддержка тёмной темы — важный аспект современного пользовательского интерфейса. Используя приведённые примеры кода и инструменты вроде MetaDarkStyle, вы сможете адаптировать свои приложения для работы с разными темами интерфейса.
