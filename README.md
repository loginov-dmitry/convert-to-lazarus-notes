...
Виджеты QT5 по умолчанию поддерживают тёмную тему в Linux (по крайней мере в Astra Linux и RedOS). Виджеты gtk2 также поддерживают тёмную тему без дополнительных манипуляций (зависит от дистрибутива Linux). Таким образом, если вы используете только стандартные компоненты и только системные цвета, то не будет никаких проблем с поддержкой тёмной темы. 

К сожалению, системных цветов недостаточно при разработке сложного приложения. В таком случае, программа должна уметь динамически выбирать правильный цвет в зависимости от текущей темы (светлой либо тёмной).

Для того, чтобы детектировать тёмную тему, используйте следующую функцию:
...
Данный способ детектирования тёмной темы является кроссплатформенным и работает с любыми виджетами. Без виджетов (например, в консольном приложении) функция также должна скомпилироваться, но что именно она покажет в этом случае - я не проверял (вероятно, результат будет всегда False).

Для каждого цвета светлой темы можно вручную подобрать подходящий цвет для тёмной темы. Учитывайте то, что TColor - это не окончательный цвет. В нём можно закодировать компоненты RGB, но располагаться эти компоненты в TColor должны в другом порядке.

Ниже приведён пример функции ConvertGUIColor, которая подбирает для заданного цвета светлой темы подходящий цвет тёмной темы. Пример нельзя назвать удачным, поскольку в нем учитываются лишь несколько цветовых констант. Но вы можете доработать её под себя.
...
В нашем проекте также используется пакет `MetaDarkStyle`, который можно установить с помощью OnlinePackageManager. Данный пакет добавляет поддержку тёмной темы в ваши приложения под Windows. Также он позволяет переключить IDE Lazarus в режим "тёмная тема". Польза пакета `MetaDarkStyle` для нашего проекта заключается в том, что мы можем сразу посмотреть, как выглядит программа в режиме "тёмная тема", без необходимости её сборки под Linux.
